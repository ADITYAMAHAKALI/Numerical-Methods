...
def build_table_modified(range_start,range_end,increment):
    X=[]
    Y=[] 
    i=0
    while(range_end>0 and range_start < 0 ):
        if(i%2==0):
            X.append(range_end)
            Y.append(func(range_end))
            range_end-=increment
        else:
            X.append(range_start)
            Y.append(func(range_start))
...
def main():
    # this function is always positive and thus we cannot find initial point x0,x1 as intermediate value theorem is never followe
    X=[]
    Y=[]
    X,Y = build_table_modified(-3,3,0.5)
    printTable(X,Y)
